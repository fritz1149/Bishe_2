...
def make_net(args):
    ...
    backbone = TrafficEmbedder(args)
    h_dim = args.linear_output_dim
...
def get_trainable_state_dict(net):
    """仅返回可训练参数的 state_dict，用于轻量化保存。"""
    model = net.module if hasattr(net, "module") else net
    full_state = model.state_dict()
    trainable_state = {}
    for name, param in model.named_parameters(recurse=True):
        if param.requires_grad:
            trainable_state[name] = full_state[name]
...
def eval(net, args):
    def _load_data(args):
        from dataset import CustomDataset, collate_ContrastiveDataset_test2
        ...
        loader_8 = torch.utils.data.DataLoader(subset_8, batch_size=args.per_device_batch_size,
                                               ...
                                               drop_last=True, sampler=sampler_8, collate_fn=collate_ContrastiveDataset_test2)
        loader_2 = torch.utils.data.DataLoader(subset_2, batch_size=args.per_device_batch_size,
                                               num_workers=args.workers, pin_memory=True,
                                               drop_last=True, sampler=sampler_2, collate_fn=collate_ContrastiveDataset_test2)
    ...
    with torch.no_grad():
        for (x, labels) in train_loader:
            # x = tuple(x_.to(args.device, non_blocking=True) for x_ in x)
...
def main(args):
    ...
    start_epoch = 0
    best_loss = float("inf")
    loss_fn = nn.CosineSimilarity(dim=-1, eps=1e-6).to(device)  # 增大eps提高数值稳定性
    net = make_net(args)
    print(net)
    if is_distributed():
        net.module.backbone.backbone.gradient_checkpointing_enable()
        net.module.backbone.backbone.enable_input_require_grads()
        print("requires grad:")
        for name, param in net.module.named_parameters(recurse=True):
            if param.requires_grad:
                print(name)
    else:
        net.backbone.backbone.gradient_checkpointing_enable() # type: ignore
        net.backbone.backbone.enable_input_require_grads()
        print("requires grad:")
    ...
    ##   DATA  ##
    #############
    from dataset import CustomDataset, LMDBDataset
    if args.lmdb_dataset:
        dataset = LMDBDataset(args.train_dir)
    else:
        dataset = CustomDataset(args.train_dir)
    sampler = torch.utils.data.distributed.DistributedSampler(dataset) if is_distributed() else None
    scaler = torch.amp.GradScaler()
    from dataset import collate_ContrastiveDataset2
    loader = torch.utils.data.DataLoader(dataset, batch_size=args.per_device_batch_size, shuffle=(sampler is None),
                num_workers=args.workers, pin_memory=True, sampler=sampler, drop_last=True, collate_fn=collate_ContrastiveDataset2)
    ...
    if not is_distributed():
        net.backbone.resume(args)
    else:
        net.module.backbone.resume(args)
    if args.resume:
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for step, (x1, x2) in enumerate(loader, start=epoch*len(loader)):
            try:
                # x1 = tuple(x.to(args.device, non_blocking=True) for x in x1)
                # x2 = tuple(x.to(args.device, non_blocking=True) for x in x2)
                ...
                # 梯度裁剪需要在 scaler.step() 之前，且需要先反缩放梯度
                scaler.unscale_(optimizer)  # 反缩放梯度
                if args.clip_grad_norm is not None:
                    torch.nn.utils.clip_grad_norm_(net.parameters(), max_norm=args.clip_grad_norm)
                
                # 检查梯度是否有nan/inf，如果有则跳过此次更新
                if any(p.grad is not None and (torch.isnan(p.grad).any() or torch.isinf(p.grad).any()) for p in net.parameters()):
                    print(f"Warning: Skipping step {step} due to nan/inf gradients (scale={scaler.get_scale():.2f})")
                    optimizer.zero_grad()
                    scaler.update()
                    # 注意：跳过时不调用scheduler.step()，避免学习率调度混乱
                    continue
        ...
        if args.is_master:
            ...
            if (epoch % args.save_freq == 0) or is_best:
                state = dict(epoch=epoch, model=get_trainable_state_dict(net), optimizer=optimizer.state_dict(), best_loss=best_loss)
...
def add_simsiam_args(parser):
    ...
    parser.add_argument('--save_freq', type=int, default=2, help="模型保存频率（每多少个epoch保存一次）")
    parser.add_argument('--epochs', type=int, default=100, help="训练轮数")
    parser.add_argument('--batch_size', type=int, default=256, help="总batch size（会被world_size整除）")
    parser.add_argument('--seed', type=int, default=None, help="随机种子")
    parser.add_argument('--lmdb_dataset', action='store_true', default=False, help="是否使用lmdb_dataset")
    ...
    parser.add_argument('--train_mode', action='store_true', default=False, help="是否启用train模式")
    ...
    parser.add_argument('--llm', type=str, default='Qwen3-VL-Embedding-2B', help="LLM模型名字")
    parser.add_argument('--projector', type=str, default='linear', help="payload Embedder的投影头类型")
    parser.add_argument('--linear_output_dim', type=int, default=2048, help="payload Embedder线性投影头的输出维度")

    # optim
    parser.add_argument('--optimizer', type=str, default='adamw', choices=['sgd', 'adamw'], help="优化器类型")
    # INSERT_YOUR_CODE
    parser.add_argument('--clip_grad_norm', type=float, default=1.0, help="梯度裁剪最大范数（None为不裁剪）")
    # common
