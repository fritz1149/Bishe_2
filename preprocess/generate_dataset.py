...
#     prompt2_ids = _str_to_ids(prompt2, type="qwen3vl")[0]
    ...
    for label in tqdm(label_names, desc="处理标签", file=sys.stdout):
        ...
        num_val = int(calc_start * 0.1)
        num_train = k_this-num_val*2
        ...
        test_pcaps = pcap_names[num_train + num_val:num_train + 2*num_val]
...
给定流的类别是："""
        ...
        system_prompt = """<|im_start|>system
你是一个AI助手，擅长阅读网络流量并对其进行思考和理解，并能够完成各种针对网络流量的问题。<|im_end|> """
        prompt = system_prompt + f"""
<|im_start|>user
接下来会给出一个五元组定义的流的流量信息，包含若干个包的头部特征和统计特征，以及加密载荷。请输出对应的类别。
类别包含: {", ".join(label_names)}。
接下来是流量信息：<流量开始>"""
        prompt2 = """<流量结束><|im_end|>
<|im_start|>assistant
    ...
    for idx, label in enumerate(label_names, 1):
        ...
        if use_catalog:
            ...
            for catalog_filename, dataset_name in [("train.txt", "train"), ("val.txt", "val"), ("test.txt", "test"), ("tgt.txt", "tgt")]:
                ...
                if n != len(pcap_names):
                    print(f"    ⚠️ 警告: 样本数量不匹配: {catalog_file} 生成{n}个样本，预期{len(pcap_names)}个")
