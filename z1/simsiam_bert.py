...
def make_net(args):
    ...
    from z1.encoder import get_longformer
...
def eval(net, args):
    def _load_data(args):
        from dataset import CustomDataset, collate_ContrastiveDataset_test
...
def main(args):
    ...
    if args.fix_pred_lr:
        ...
        optims_params = [{'params': model.backbone.parameters(), 'fix_lr': False},
                        {'params': model.predictor.parametersmodel(), 'fix_lr': True}]
    else:
    ...
    from dataset import CustomDataset, collate_ContrastiveDataset
