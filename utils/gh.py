...
def _flatten(grads):
    import torch
    parts = [g.reshape(-1) for g in grads] # grads一定没有None
...
def GH(grads1, grads2):
    """grads1, grads2: 1D 扁平化梯度张量"""
    import torch
    g1 = grads1
    g2 = grads2
    dot = torch.dot(g1, g2)
    if dot < 0:
        eps = torch.tensor(1e-8, dtype=g1.dtype, device=g1.device)
        norm1_sq = g1.norm() ** 2 + eps
        norm2_sq = g2.norm() ** 2 + eps
...
def GH_plus(grads1, grads2, lam=0.5):
    ...
    if dot < 0:
        ...
        norm1 = g1.norm()
        norm2 = g2.norm()
        cos_sim = (dot / (norm1 * norm2 + eps)).clamp(-1.0, 1.0)
